...
def load_models(checkpoint_dir, device):
    ...
    if checkpoint is None:
        ...
        print(
            f"⚠️ Checkpoint '{checkpoint}' not found!"
            "Using untrained compressor weights for test."
        )
...
class ILDC(nn.Module):
    ...
    def kv_compress(
        ...
        # The latent-states and compressed-kvs will be collect on each diffusion steps and
        # will be given to AR model for student generation pass
        # Pre-allocate tensors Shape: (Steps, Batch, K_latent_len, Latent)
        all_latent_states = torch.empty(
            (diff_steps, B, K_latent_len, latent_size), device=device, dtype=target_dtype
        )
        ...
        for t in range(diff_steps):
            ...
            # Dit Model Pass
            # h: (B, K_latent_len, latent_size),
            # kv: (B, layers, K_latent_len, 2, heads, head_dim)
            latent_states, latent_kvs = self.dc_model(
                latent_states, latent_kvs, safe_kv_caches, timestep, start_pos
            )
    ...
    def bptt_forward(
        ...
        # Our start-pos for compression will start after the cmpressed kvs for efficient
        # compression, if both the compressed_kv and start-pos are given,...
        # we assume is as mistake and overwrite the start-pos
        ...
        with torch.no_grad():
            ...
            # total uncompressed kvs, from input kV_caches and the new uncompressed context KVs
            # to be compressed
            uncomp_context_kvs = teacher_active_kv_caches[:, :, : int(eff_seq_len), ...]

        # KV Compression
        # Totals KVs on which to conditioned for compression: Compressed_KVs and
        # Uncompressed KVs too!
        if active_compressed_kv is not None:
        ...
        if active_compressed_kv is not None:
            T_active_compressed_kv = active_compressed_kv.unsqueeze(1).expand(
                B, T, L, CompKV_len, 2, H, D
            )
            T_compressed_kv_caches = torch.cat(
                [T_active_compressed_kv, T_newcompressed_kv_caches], dim=3
            )
        else:
        ...
        # T times with each step having its own compressed_kv_caches for different steps of
        # same forward pass.
        ...
        train_output = {
            "teacher": {"hidden": h_teacher, "logits": logits_teacher},  # (B, S, V)
    ...
    def forward(
        ...
        with torch.no_grad():
            ...
            # total uncompressed kvs, from input kV_caches and the new uncompressed context
            # KVs to be compressed
            uncomp_context_kvs = teacher_active_kv_caches[:, :, : int(eff_seq_len), ...]
        ...
        if active_compressed_kv is not None:
            ...
            if start_pos is not None:
                print(f"Overwriting start-pos with Comp_KV seq len: {CompKV_len}!")
            start_pos = CompKV_len
            # Totals KVs on which to conditioned for compression: Compressed_KVs
            # and Uncompressed KVs too!
            cond_context_kvs = torch.cat([active_compressed_kv, uncomp_context_kvs], dim=2)
        else:
            cond_context_kvs = uncomp_context_kvs
        ...
        # Compress the extracted KVs using the DM tower
        # we assume that during training, the previous steps has been learned before
        # start-step which can give us ground latent values..
        if start_step > 0:
        ...
        train_output = {
            "teacher": {"hidden": h_teacher, "logits": logits_teacher},  # (B, S, V)
            "student": {"hidden": h_student, "logits": logits_student},  # (B, S, V)
        }
    ...
    def generate_text(self, prompt, max_new_tokens=1024, temperature=0.7, top_k=50, top_p=0.9):
        ...
        if input_len[1] >= 1024:
            with torch.no_grad(), torch.autocast(device_type=device.type, dtype=torch.bfloat16):
                ...
                start_step = 0
                latent_states = torch.randn(
                    1, K_latent_len, 1152, device=device, dtype=torch.bfloat16
                )

                # Compress the extracted KVs using the DM tower
                T_latent_states, T_compressed_kvs = self.kv_compress(
                    latent_states, uncompressed_kvs, start_step, 2, 0
                )
        ...
        for _ in range(max_new_tokens):
            ...
            if 0.0 < top_p < 1.0:
                ...
                cumulative_probs = torch.cumsum(
                    F.softmax(sorted_logits, dim=-1, dtype=torch.float32), dim=-1
                )
...
which then going to look at the LLM KVs and produce best latent KVS as compressed KVS to be
used by LLM for further autoregression;

I doubt it!

Instead; we should keep the prev. step latent KVs and tell the diffusion model to reason
onto itself what he know and what he need to know more which will produce best KVs
to act as compressed KVs...
