...
# put a head on latent-states with stop/continue binary prediction...if they say stop.. stop else continue..till the max diffusion steps....
...
class CompDiTModel(nn.Module):
    ...
    def forward(self, latent_states, Latent_KVs, kv_caches, timestep, start_pos=0):
        ...
        for i, layer in enumerate(self.comp_layers):
            ...
            latent_states, latent_kv = layer(latent_states, Latent_KVs[:, i, ...], t_emb, position_ids, kv_caches[:, i, ...])
            Latent_KVs[:, i] = latent_kv

        return latent_states, Latent_KVs  # as we use latent kvs as compressed kvs for the ar_model 
